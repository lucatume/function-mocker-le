...
class GenerateSystem extends Command {
    ...
    public function execute(InputInterface $input, OutputInterface $output) {
        ...
        return $this->printSystem($input->getArgument('system'), $input->getOption('system-path'),
    ...
    protected function printSystem($systemName, $systemPath, array $functionsAsts) {
        ...
        $functionsFileName = "{$systemClass}-functions.php";
        $outputFunctionsFilePath = rtrim($systemPath, DIRECTORY_SEPARATOR) . '/' . $functionsFileName;
        ...
        $classStmt =
            $builder->class($systemClass)
                    ...
                    ->addStmt($builder->method('setUp')
                                      ...
                                      ->addStmt(new Include_(new Concat(new Scalar\MagicConst\Dir(),
                                          new String_('/' . $functionsFileName)),
                                          Include_::TYPE_INCLUDE_ONCE))
                    )
                    ->addStmt($builder->method('tearDown')
                                      ->makePublic()
                                      ->addStmt(new FuncCall(new Name('\\tad\\FunctionMockerLe\\undefineAll'),
                                          [new MethodCall(new Variable('this'), new Name('defined'))])))
                    ->addStmt($builder->method('defined')
                                      ->makePublic()
                                      ->addStmt(new Return_(new Array_($this->defined))));
    ...
    protected function wrapInFunctionExistsCheck($asts) {
        return array_map(function (Function_ $functionNode) {
            return new If_(new BooleanNot(new FuncCall(new Name('function_exists'),
    ...
    protected function configure() {
        $this->setName(self::NAME)
             ...
             ->addArgument('system', InputArgument::REQUIRED,
                 'The name, including namespace, of the System class to generate')
             ->addOption('system-path', 'sp', InputOption::VALUE_OPTIONAL,
                 'The path to the folder that will contain the System file; if the destination does not exist it will be created.',
                 getcwd())
